;(function(window,document) {
	...
	window[fnName] = function(setConfig){
		...
		if(_default.songSelect) {
			songSelect = _default.songSelect;
		}
		//报错信息
		...
		this.loadFile = function(playStatus){
			...
			if(song['src']){
				...
				var audioType = song['src'].substring(song['src'].lastIndexOf(':') + 1,song['src'].lastIndexOf(';'));

				//判断是否支持当前音频格式
				if(control.canPlayType(audioType)){
		...
		this.close = function(){
			elem.songTitle.innerText = "";
			control.src = "";
			elem.thisTime.innerText = "00:00";
			elem.countTime.innerText = "00:00";
